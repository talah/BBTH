	...
	public void compute() {
		...
		while (iter.hasNext()) {
			...
			for (int i = 0, wallCount = walls.size(); i < wallCount; i++) {
				...
				float t_edge_on_wall = ((edge.p1.x - wall.a.x) * wallY - (edge.p1.y - wall.a.y) * wallX) / divide;
				float t_wall_on_edge = ((wall.a.x - edge.p1.x) * edgeY - (wall.a.y - edge.p1.y) * edgeX) / -divide;

				// If they overlap (expanding the wall out to include the entity
				// radius), remove the edge
				float padding = radius * 0.95f / wall.length;
				if (t_edge_on_wall >= 0 && t_edge_on_wall <= 1 && t_wall_on_edge >= 0 - padding && t_wall_on_edge <= 1 + padding) {
					iter.remove();
					break;
				}
			}
