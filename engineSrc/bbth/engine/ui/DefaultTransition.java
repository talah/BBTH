...
public class DefaultTransition extends Transition {
	...
	public DefaultTransition(float length) {
		this(length, new Envelope(255f, Envelope.OutOfBoundsHandler.RETURN_FIRST_OR_LAST), new Envelope(255f, Envelope.OutOfBoundsHandler.RETURN_FIRST_OR_LAST), Envelope.ALWAYS_ZERO, Envelope.ALWAYS_ZERO, Envelope.ALWAYS_ZERO, Envelope.ALWAYS_ZERO, Envelope.ALWAYS_ONE, Envelope.ALWAYS_ONE);
	}
	
	public DefaultTransition(float length, Envelope newAlpha, Envelope oldAlpha, Envelope newX, Envelope oldX, Envelope newY, Envelope oldY, Envelope newScale, Envelope oldScale) {
		this.length = length;
	...
	private void drawLayer(UIView view, Canvas canvas, Envelope alpha, Envelope x, Envelope y, Envelope scale) {
		canvas.getClipBounds(clipBounds);
		int restoreNumber = canvas.saveLayerAlpha(0f, 0f, (float)canvas.getWidth(), (float)canvas.getHeight(), (int)Math.round(alpha.getValueAtTime(currentTime)), Canvas.ALL_SAVE_FLAG);
		...
		canvas.clipRect(0f, 0f, scaleAmount*clipBounds.width(), scaleAmount*clipBounds.height());
