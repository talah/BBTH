 ...
 * 
 * @author Zach Davis
 * @param <E> the type of elements this Bag contains
 * @see Collection
 * @see Set
 * @see List
 */
public class Bag<E> extends AbstractCollection<E> implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 * 
	 * @param collection the collection from which to add all the elements
	 */
	 ...
	 * 
	 * @param numNewElements the number of new elements to ensure the
	 *            <tt>Bag</tt> has room for
	 */
	...
	// Added because Android's java.util.Arrays sucks (doesn't have the copyOf method)
	static <T> T[] copyArray(T[] original, int newLength) {
		@SuppressWarnings("unchecked")
		T[] newArray = (T[]) ((original.getClass() == Object[].class) ? new Object[newLength] : Array.newInstance(original.getClass().getComponentType(), newLength));
		System.arraycopy(original, 0, newArray, 0, original.length < newLength ? original.length : newLength);
		return newArray;
	}
}
