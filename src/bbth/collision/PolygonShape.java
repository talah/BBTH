...
public class PolygonShape extends Shape {
	...
	@Override
	public boolean isCrossingLineSegment(float originX, float originY, float rayX, float rayY) {
		for (int i = 0, j = worldPoints.length - 1; i < worldPoints.length; j = i++) {
			...
			if (denom != 0) {
				float t1 = (rayX * (iy - originY) - rayY * (ix - originX)) / denom;
				float t2 = ((jx - ix) * (iy - originY) - (jy - iy) * (ix - originX)) / denom;
	...
	private void applyTheoremToCircle(SeparatingAxisTheorem theorem, CircleShape other) {
		for (int i = 0; i < worldPoints.length; i++) {
			theorem.changeAxis(other.worldTransform.offset.x - worldPoints[i].x, other.worldTransform.offset.y - worldPoints[i].y);
	...
	private void applyTheoremToSimpleShape(SeparatingAxisTheorem theorem, Shape other) {
		for (int i = 0, j = worldPoints.length - 1; i < worldPoints.length; j = i++) {
			theorem.changeAxis(worldPoints[i].y - worldPoints[j].y, worldPoints[j].x - worldPoints[i].x);
			recordPoints(theorem);
			theorem.storeRange();
			other.recordPoints(theorem);
			theorem.compareRanges();
